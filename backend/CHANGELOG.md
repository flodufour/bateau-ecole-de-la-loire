...
2026-09-02 19:00 — [feat] Let an Admin also act as instructor (confirmed real case: the school's admin gives the courses) — POST /instructors/me links a profile to the caller's own account, and the instructor-self endpoints now accept Instructor or Admin, instead of building general multi-role support for one case — with full test coverage
2026-09-02 21:30 — [feat] Add permit purchases (new `permit_purchases` table): POST/GET /api/purchases lets a student buy a permit and list their own purchases, and POST /api/purchases/{id}/transfer moves one to another registered account by email (one person buying for another) — no real payment yet, a purchase is created already paid — with full test coverage
2026-09-02 22:30 — [feat] Replace the single-permit purchase endpoint with POST /api/purchases/checkout, taking a cart of { permitId, quantity } items (1-20 each) and creating one purchase row per unit — supports the new cart system, each unit stays individually transferable — with full test coverage
