...
Also embeds the school's Facebook page (Meta's iframe-based Page Plugin, `facebook.com/plugins/page.php?href=...`) below the contact info, in a bordered card under a "Suivez-nous sur Facebook" heading. The URL is a hardcoded constant (the page id), never user input, so bypassing Angular's `iframe[src]` sanitization with `DomSanitizer.bypassSecurityTrustResourceUrl` is safe here — same reasoning as the Google Map on the home page. Deliberately **not** passing `adapt_container_width` — that flag is for the JS-SDK (`fb-page` div) embed method, which resizes the iframe itself; on this plain fixed-size iframe embed it made Facebook render content wider than the box and clip text. `small_header=true` and `show_facepile=false` keep it compact.
...
- Both forms on this page (profile creation and availability) show "Veuillez remplir tous les champs." when submitted incomplete, instead of `markAllAsTouched()`'s silent no-op.
...
Every create/edit form here (and the instructor portal's, see below) shows "Veuillez remplir tous les champs." via the same `errors` signal used for backend errors when submitted while still `invalid` — `markAllAsTouched()` alone was a silent no-op with no visible feedback.
...
- **Sessions** (`/admin/seances`) — full CRUD. Form has `<select>`s for permit and instructor (fetched on init alongside the sessions list); `startsAt` uses a `datetime-local` input, converted to/from ISO 8601 at the form boundary. Delete is rejected if the session has bookings. The "Moniteur" `<select>` marks the caller's own instructor profile (via `GET /instructors/me`, silently ignored if 404 — most Admins don't teach) with "(moi)" after their name.
