...
- **Ambient wave background**: `WaveBackground` (`shared/components/wave-background/`, mounted once in `app.html`) draws up to 4 small orange wavy SVG lines that each fade in, "draw" themselves via an animated `stroke-dashoffset`, hold for ~2s, then fade out and reappear at a new random spot roughly 9–11s later. It's `position: fixed` at `z-index: -1` behind the whole app, so the opaque navy sections (header, footer, hero bands) naturally paint over it — the waves only ever show through the plain white areas of a page, with no per-page targeting needed. Skips the whole animation loop (not just the CSS) for `prefers-reduced-motion: reduce`.
