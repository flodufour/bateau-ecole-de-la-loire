...
2026-09-03 00:00 — [feat] Add `WaveBackground`, an ambient decoration mounted app-wide: up to 4 small orange wavy SVG lines fade/draw in, hold ~2s, then fade out and reappear elsewhere ~10s later — `position: fixed` at `z-index: -1` so it only shows through plain white areas, not the navy header/footer/hero bands; skips the animation loop entirely under `prefers-reduced-motion` — with tests
